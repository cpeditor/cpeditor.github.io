...
Each rule is a pair of [regular expression patterns](../general/\_index.zh_tw.md#正規表達式). The first regex matches the whole input file name, and the second pattern is a replacement pattern which is the name of the answer file.
...
Each rule is a pair of [regular expression patterns](../general/\_index.zh_tw.md#正規表達式). The first regex matches a part of the problem URL, and the second pattern is a replacement pattern which is the default save path.
